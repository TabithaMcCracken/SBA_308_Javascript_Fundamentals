...
function calculateGradePercentage(
  ...
  try {
    if (studentGrade >= 0 && studentGrade <= pointsPossible) {
      ...
      throw new Error(
        `The assignment ${submission.assignment_id} for the student with id ${submission.learner_id} is is not a valid grade and won't be included in the grade totals.`
      );
    }
  } catch (error) {
    ...
    throw new Error("Invalid input: the course id does not match.");
      ...
      try {
        ...
        for (let i = 0; i < submissions.length; i++) {
          ...
          const score = parseInt(submission.submission.score); // Score the student received
          const pointsPossible = ag.assignments.find(
            (a) => a.id === assignmentId
          ).points_possible; // Find assignment object with matching ID, then access points_possible

          const dueDate = ag.assignments.find((a) => a.id === assignmentId).due_at; // Find assignment with matching id
          ...
          if (
            ...
            studentData[existingStudentIndex].totalGrade +=
              gradePercentage * pointsPossible;
            ...
            studentData[existingStudentIndex][assignmentId] = parseInt(
              gradePercentageNumber.toFixed(2)
            );
        ...
        studentData.forEach((student, index) => {
          if (student.validAssignmentCount >= 1) {
            const avg = Math.round(
              student.totalGrade / student.totalPointsPossible
            );
